...
\fbox{2.8} Vous pouvez assigner un style de page à des \trad{niveaux de classe} |top| 
ou |page|, aussi bien qu'au chapitre de base avec la commande 
suivante\footnote{Nommée dans l'éphémère version 2.7 \texttt{\string\titlepagestyle}.} :
\begin{desc}
...
\trad{\verb|top|}. Les valeurs admises sont \verb|\cleardoublepage| (valeur par 
défaut avec \verb|openright|) et \verb|\clearpage| (valeur par défaut avec 
\verb|openany|). Ainsi, vous pouvez annuler l'effet de \verb|openright| en définissant \verb|\chapterbreak| comme \verb|\clearpage|, du moment que sa 
...
\fbox{2.7} Avec cette option, le titre n'est pas implicite à la fin de |<code-avant>| mais doit être indiqué explicitement avec |#1| comme dans, par exemple :
\begin{verbatim} 
\titleformat{\section}
 {..}
 {\thesection}{..}{#1.}
...
dispose d'une syntaxe étendue qui permet de \trad{fixer} différentes mises en forme
dépendant du contexte\footnote{L'extension \textsf{keyval} est requise pour pouvoir utiliser cette syntaxe.}. Cet argument peut être une liste de paires clé/valeur de la forme :
\begin{desc}
...
\begin{itemize}
\item |name|. Les valeeurs possibles sont |\chapter|, |\section|, etc.
...
(autrement dit celles avec un niveau sous |secnumdepth|, dans la \trad{front and back 
matters} et, bien entendu, la version étoilée). Par exemple
\begin{verbatim}
...
\subsection{Création de nouveaux niveaux et changement de classe}

\tradini While the shapes and the like modify the behaviour of titles related
