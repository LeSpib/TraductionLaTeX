...
pt\footnote{Pour vous aider, de nombreux prévisualisateurs de dvi permettent 
...
Comme décrit plus haut, mais avec différents |<format>|s. Les valeurs par
...
est similiaire à la forme |hang| de \textsf{titlesec}.
...
celui est toujours présent; d'autre part, |<begin>| est rarement utilisé et est
...
paramètres |<séparateur>| et |<end>| (dont les valeurs par défaut sont des vides) 
...
\tradini At the point where this command is used, a partial toc begins (note
the document doesn't require a |\tableofcontents| for partial tocs to
work).  The |<nom>| argument allows different sets of tocs and it
...
Stops the partial toc of |<nom>| kind, which may be resumed.
...
Print the current partial toc of |<nom>| kind. The format
of the main toc entries are used, except if there is a |<préfixe>|.
In such a case, the format of |<préfixe><niveau>| is used, provided
...
the format is that of |section|. The |<niveau-début>| parameter sets the
top level of the tocs---for a part toc it would be |0| (chapter), for a
chapter toc |1| (section), and so on. Finally, |<code-table>| is
...
Here |<liste>| is either |lof| or |lot|.  Note as well |\printlist|
does not have the |<début-niveau>| argument, because figures and tables
...
|<code-table>| with |\renewcommand\addvspace[1]{}|.
