...
En définissant ces commandes avec |\newcommand|, différents sauts de page peuvent
...
mais doit être indiqué explicitement avec |#1| comme dans l'exemple suivant :
\begin{verbatim} 
\titleformat{\section}
 {..}
 {\thesection}{..}{#1.}
...
classe |article| mais vous ne devriez pas l'utiliser du tout. Utilisez la classe
|straight| à la place. Rappelez-vous que certaines fonctionnalités reposent sur
ces classes et que \textsf{titlesec} ne change pas par défaut les définitions
de \texttt{\string\part} et \texttt{\string\chapter}.}. 
\end{itemize}
...
niveaux en-dessous sont incrémentés de 1 (la section devient 2, la sous-section 3
et ainsi de suite).
...
\begin{itemize}
\item |\chapter| récupère la chaîne définie dans |\chaptertitlename| et la prend
 en compte ;
...
doivent être composées en petits caractères, ajoutez un |\small|, et si vous réalisez que vous n'aimez pas cela, retirez-le.

Tandis que les commandes standard de \LaTeX{} sont plus faciles et directes 
pour les cas simples, je pense que la méthode proposée ci-dessus est de loin préférable pour les longs documents.
...
Ici:
\begin{itemize}
\item |<section>| est le nom de la commande de sectionnement sans l'antislash 
