...
\title{L'extension \textsf{ulem}\thanks{Ce fichier a pour numéro de
        version 2012-05-18 et a été mis à jour le \filedatefr. Son 
...
\begin{abstract}
  L'extension \textsf{ulem} fournit différents types de soulignement qui
  peuvent s'étendre entre les mots et peuvent être brisés entre plusieurs
  lignes. Il s'utilise avec \LaTeX\ ou plain \TeX.

  En LaTeX, \textsf{ulem} remplace normalement l'italique par le soulignement
  dans le texte mis en valeur par \cs{emph} et, dans une certaine mesure, par
  \cs{em}. La déclaration de |\normalem| ou l'option |[normalem]| pour 
  |\usepackage| annulent cette fonctionnalité.

  Les commandes suivantes sont définies pour une general use:\\[5pt]
  \indent \begin{tabular}{l@{\quad}l}\hline\noalign{\vskip2pt}
   |\uline{important}|  & soulignement simple tel \uline{important}\\[1pt]
   |\uuline{urgent}|    & soulignement double tel \uuline{urgent}\\[1pt]
   |\uwave{boat}|       & soulignement ondulé tel {\let\ULleaders\cleaders\uwave{boat}}\\[1pt]
   |\sout{wrong}|       & texte barré tel \sout{wrong}\\[1pt]
   |\xout{removed}|     & texte hachuré tel \xout{removed} \\[1pt]
   |\dashuline{dashing}|& soulignement par tirets tel \dashuline{dashing}\\[1pt]
   |\dotuline{dotty}|   & soulignement par pointillés tel \dotuline{dotty}\\[3pt]\hline
  \end{tabular}\\[6pt]
   D'autres commandes peuvent être définies assez simplement en utilisant la
   commande \cs{markoverwith} fournie avec \textsf{ulem}.
\end{abstract}

\begin{footnotesize}
\noindent L'extension \textsf{ulem} est sous copyright \copyright\ 1989--2011 
par Donald Arseneau (Vancouver, Canada).\\
L'extension (ulem.sty) et sa documentation (ulem.ltx, ulem.pdf) peuvent être
gratuitement transmis, reproduits ou modifiés pour tout objectif tant que 
la notice de copyright est gardée intacte.
(De courts extraits peuvent bien entendu être pris et utilisés sans aucune
restriction.)\par
\end{footnotesize}
